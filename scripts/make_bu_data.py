...
# output_dir
parser.add_argument('--downloaded_feats', default='/media/rubick/part2/datasets/data/UpDown-Feature', help='downloaded feature directory') #path to updown feature
...
os.makedirs(args.output_dir+'_att_36')
os.makedirs(args.output_dir+'_fc_36')
os.makedirs(args.output_dir+'_box_36')
cocobu_size = {}
...
for infile in infiles:
    ...
    with open(os.path.join(args.downloaded_feats, infile), "r+b") as tsv_in_file:
        ...
        for item in reader:
            ...
            np.savez_compressed(os.path.join(args.output_dir+'_att_36', str(item['image_id'])), feat=item['features'])
            np.save(os.path.join(args.output_dir+'_fc_36', str(item['image_id'])), item['features'].mean(0))
            np.save(os.path.join(args.output_dir+'_box_36', str(item['image_id'])), item['boxes'])
            cocobu_size_temp = {}
            cocobu_size_temp['image_w'] = item['image_w']
            cocobu_size_temp['image_h'] = item['image_h']
            cocobu_size[str(item['image_id'])] = cocobu_size_temp
save_path = 'data/cocobu_size.npz'
