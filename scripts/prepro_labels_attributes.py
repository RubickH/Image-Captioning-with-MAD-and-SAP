...
def build_vocab(imgs, params):
    ...
    print('\n'.join(map(str,cw[:20])))
    #these top words are not considered as attributes
    except_vocab = ['a','on','of','the','in','with','and','is','to','an','at','are','next','that','it','up','his'\
                    ,'by','there','other','for','out','her','from','their','its','as','this','into','another'\
                    ,'being','off','what','where','when','why','who','whose','how']
    # print some stats
    ###first place the top 1000 attributes!
    vocab = []
    for n,w in cw:
        if(n > count_thr and w not in except_vocab):
            vocab.append(w)
